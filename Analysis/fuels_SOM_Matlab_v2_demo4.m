...
try 
  sD = som_read_data('clean_data_22-tstcar-2022-04-15_v3_tabDelimitedD10_initialized.txt');     
catch
  echo off
...
%sM = som_make(sD, 'msize', [12 20], 'shape', 'cyl'); %cylindrical topol
sM = som_make(sD, 'msize', [15 12], 'shape', 'sheet'); %cylindrical topol
%sM = som_make(sD, 'msize', [12 20], 'shape', 'toroid'); %toroid topol
%sM = som_make(sD);
...
som_show(sM,'umat','all','comp','all','norm','d');
...
subplot(1,3,1)
[c,p,err,ind] = kmeans_clusters(sM, 10); % find at most 7 clusters
plot(1:length(ind),ind,'x-')
[dummy,i] = min(ind);
cl = p{i};

% save the clustering metrics as CSV file
k_index = 1:length(err);
clustering_metrics = cat(2,k_index.',err.',ind.');
csvwrite('2022_Clustering_metrics_clusters10max_D1_REC_v5_2.csv',clustering_metrics)
...
%% Consolidating BMUs, clusters, original samples and variables data 

bmus_cl = bmus;
emptyCol = zeros(length(bmus_cl),1);
sample_index = (1:length(bmus_cl)).';
bmus_cl_table = cat(2,sample_index,bmus_cl,emptyCol);

for i = 1:length(bmus_cl)

    bmu = bmus(i);
    cl_i = cl(bmu);
    bmus_cl_table(i,3) = cl_i;

end

% Combine the BMU and clusters data with the denormalized SOM data for all
% variables and samples

sample_bmus_cl_table = cat(2,bmus_cl_table,D);

% save the data as CSV file for external analysis
csvwrite('2022_samples_BMUs_clusters10max_D10_REC_v5_2.csv',sample_bmus_cl_table)
